...
//avoid, add to .hpp later
vector<Robot> adversaries;

//Average error real time average for display
double sim_ave_error	= 0;
double sim_ave_error_2  = 0;
int rounds 				= 0;
int epochs 				= 1;
...
int main(int argc, char **argv){
	...
	for(int i = 0; i < NUM_ADVERSARIES; i++){
		Robot robot(i, 0.0, 10, 10, 0, ADV_VEL, ADV_STEERING, shape, &adversaries, 50, 0, 0, false, 0);
...
void spawn_world(){
	...
	for(auto const &r : flock){
		double rx = origin_x + ((double)rand()/RAND_MAX-0.5)*ROBOT_SPAWN_RNG;  //ADDED *25
	...
	goals.clear();
	obstacles.clear();
	double th = rand() % 360 * PI/180;		//goal angle
	for (int i = 0; i < NUM_GOALS; i++){
		goals.push_back(pair<int, int>(sin(th) * GOAL_SPAWN_RNG, cos(th) * GOAL_SPAWN_RNG));
	}
	//avoid
	for(auto const &r : adversaries){
		double rx = sin(th)*GOAL_SPAWN_RNG/2 + ((double)rand()/RAND_MAX-0.5)*ROBOT_SPAWN_RNG;
		double ry = cos(th)*GOAL_SPAWN_RNG/2 + ((double)rand()/RAND_MAX-0.5)*ROBOT_SPAWN_RNG;
...
//comments:
/*
5-29-18: it is working best when there is a heavy reward for non leaders reaching the goal rather
than heavy punishment when they get eaten. it learns better this way. attempting a 500
epoch 4 10 500 1 experiment
-results: sometimes it non-leaders get hurded to the goal well, but the leaders have a very
hard time keeping them at the goal, or really hearding them in general. should try more leaders
and more agents maybe
-the moment the leaders get the agents to the goal they lose their hearding strategy and spin
very fast which results in losing the agents
-raised leader count and am getting negative errors now.... (was a screen cutoff issue my b)
-error needs to be scaled better to account for different numbers of agents, b/c its 
not learning well enough atm
5-30-18: fixed a file saving error for the weights and now it learns very well with 6 leaders
and 10 agents without any positive feedback for non-leaders reaching the goal
-very large difference between 6 leaders and 4 leaders with 10 agents
-the moment I put positive feedback for non-leaders reaching goal with same weight the sim
did much better
*/
double compute_error(){
	double error = 0.0;
	...
	for(auto const &r : flock){
		if(!(r.leader))
			error += r.acc_dist; 						  //measures non-leader's distance to goal
		if(r.eaten)
			num_eaten += 1;								  //counts number of eaten robots
		if(!(r.leader) && r.is_within_goal_radius(flags) && !r.eaten) //only rewarded if not eaten and makes it to goal 
			count++;									  //counts follower robots at goal	
	}

	error /= (num_robots-num_leaders);
	error /= EPOCH_STEPS;									//scales error due to distance from goal
	error += ((20*num_eaten) / (num_robots-num_leaders));	//punishes eaten
	error -= ((count * 30) / (num_robots-num_leaders));	    //rewards when non-leader robots reach goal

	//runtime error viewing below:------------------------ 
	cout << "error " << error << "\n";

	sim_ave_error += error;
	rounds++;
	if(rounds == 14){							//end of epoch
		sim_ave_error /= rounds;				//shows rolling average error to user through that epoch
		cout << "sim ave error for that epoch: " << sim_ave_error << "\n";
		rounds = 0;
		sim_ave_error_2 += sim_ave_error; 		//entire sim rolling average
		cout << "sim ave error entire runtime: " << (sim_ave_error_2 / epochs) << "\n";
		epochs +=1;								//increment counter to compute average
	}
	//end runtime error viewing:----------------------------
	//handles negative error values
	if(error < 0){
		error = 0;
	}
	return error;
