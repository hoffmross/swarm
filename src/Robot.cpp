...
//avoid, add to .hpp later, might need to be edited
void Robot::adv_update(){
	#if ENABLE_TRAIL 
		update_trail();
	#endif

	//aviod goal by stopping adversary---------------------------
	//not the right goal, this is center i think:
	//int dist_to_goal = distance_to_point(goal); //not sure if this is needed
	//cout << "dist to goal: " << dist_to_goal << "\n"; //debug
	
	//I WANT THE ADV TO SIMPLY AVOID THE GOAL AS IF IT WERE A WALL

	if(distance_to_point(goal) < 100){   //PLAY WITH THIS NUMBER
		//repulsion
		//this->v = 0;
		// Repulsion
		// Vector from goal to my position
		//pair<double, double> rep = (this->x - goal.first, this->y - goal.second);
		// Update heading towards vector direction
		double g = atan2(this->y - goal.second, this->x - goal.first)*180/PI;
...
pair<double, double> Robot::leader_reasoning(vector<Textured> &obstacles){
		...
		if (NUM_OBSTACLES != 0){
			...
			mlp->x[4] = adv_vec.first/WORLD_SIZE_X;
			mlp->x[5] = adv_vec.second/6.28;
		}
		mlp->eval();
...
void Robot::render_robot(){
	if(leader)
		...
		render(1,selected,0.3,0.3,0.3); //gray
	else
		render(1,selected);
};
